...
public class Lanzar : MonoBehaviour
{
    ...
    public float velocidadCarga = 1.5f; // Qu� tan r�pido sube la barra
    ...
    public void OnApuntar(InputValue Value)
    {
            ...
            && !GameManager.instancia.menuPrincipal && GameManager.instancia.turnoJugador)
        {
            ...
            if (ball == null)
            {
                prefabBall.transform.localScale = new Vector3(4f, 4f, 4f); // Ajusta el tama�o de la bola
    ...
    {
        // Esto crea un efecto de "vaiv�n" (sube y baja) si se mantiene mucho tiempo
        if (subiendo)
        {
